...
use App\Http\Controllers\ImportTranslationsController;
...
// Add these routes where other admin routes are located
Route::controller(MarketplaceOrderController::class)->middleware('auth', 'admin', 'verified', 'activity')->prefix('admin/marketplace')->group(function () {
    Route::get('/orders', 'index')->name('admin.marketplace.orders');
    Route::get('/orders/{id}', 'show')->name('admin.marketplace.order.show');
    Route::post('/orders/{id}/status', 'updateStatus')->name('admin.marketplace.order.update_status');
    Route::get('/statistics', 'salesStatistics')->name('admin.marketplace.statistics');
    Route::get('/orders/cancelled', 'cancelledOrders')->name('admin.marketplace.orders.cancelled');
    Route::get('/orders/pending', 'pendingOrders')->name('admin.marketplace.orders.pending');
});

// Импорт переводов
Route::controller(ImportTranslationsController::class)->middleware('auth', 'verified', 'activity', 'admin')->group(function () {
    Route::get('admin/import/translations', 'index')->name('admin.import.translations');
    Route::post('admin/import/translations', 'importTranslations');
});

// Установка языка по умолчанию
Route::get('admin/set-default-language', [SetDefaultLanguageController::class, 'setDefaultLanguage'])
    ->name('admin.set.default.language');
