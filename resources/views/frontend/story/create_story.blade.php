    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{route('create_story')}}" class="text-form text-story-form" method="POST">
                @Csrf
                <textarea name="description" onkeyup="$('.input-prev-text').text($(this).val())" placeholder="What did you learn today John?" spellcheck="false"></textarea>
...
<div class="d-flex story-buttons gap-3 mt-4">
    <a href="javascript:void(0)" onclick="createStory('text-story-form');" class="btn common_btn text-story-submit">{{get_phrase('Share to story')}}</a>
    <a href="javascript:void(0)" onclick="createStory('file-story-form');" class="btn common_btn file-story-form d-hidden">{{get_phrase('Share to story')}}</a>
    <a href="javascript:void(0)" class="btn common_btn" data-bs-dismiss="modal" aria-label="Close">{{get_phrase('Discard')}}</a>
</div>