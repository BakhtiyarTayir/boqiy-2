<link rel="stylesheet" href="{{ asset('assets/frontend/css/nice-select.css') }}">
    <form class="ajaxForm event-form form_sel" action="{{ route('event.store') }}" method="POST" enctype="multipart/form-data" >
        @csrf
        @if (isset($group_id))
        ...
        <div class="entry-header d-flex mb-10 justify-content-between">
            ...
            <div class="post-controls dropdown">
                <select name="privacy" id="privacy" class="form-control select bg-secondary">
        ...
        <div class="form-group mt-3">
            ...
            <input type="text" class="form-control" name="eventname" placeholder="Enter your event name">
        </div>
        <div class="row">
            <div class="form-group col-md-6 col-lg-6 col-sm-12">
                <label for="#">{{ get_phrase('Event Date') }}</label>
                <input class="form-control" type="date" name="eventdate" placeholder="Event Date">
            </div>
            <div class="form-group col-md-6 col-lg-6 col-sm-12">
                <label for="#">{{ get_phrase('Event Time') }}</label>
                <input class="form-control" type="time" name="eventtime" placeholder="Event Time">
            </div>
        </div>
        <div class="form-group">
            <label for="#">{{ get_phrase('Location') }}</label>
            <input class="form-control" type="text" name="eventlocation" placeholder="Enter your location">
        </div>
        <div class="form-group">
        ...
        <div class="inline-btn mt-5">
            <button class="btn common_btn w-100" type="submit" onclick="document.getElementById('description').value=CKEDITOR.instances.description.getData(); CKEDITOR.instances.description.destroy()">{{get_phrase('Create Event')}}</button>
        </div>
    </form>  
