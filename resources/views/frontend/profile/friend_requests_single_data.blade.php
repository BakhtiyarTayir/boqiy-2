@foreach($friend_requests as $friend_request)
	...
	<div class="single-item-countable w-100 " id="friendRequest{{$requester_user_data->id}}">
	    <div class="card request_f bg-white">
	        <div class="f_images"><img src="{{get_user_image($requester_user_data->photo, 'optimized')}}" class="rounded img-fluid" alt=""></div>
	        <div class="card-details">
	            <h6><a href="{{route('user.profile.view', $requester_user_data->id)}}">{{$requester_user_data->name}}</a></h6>
	            ...
	            <div class="card-control d-flex">
	            	<form class="ajaxForm" action="{{route('profile.accept_friend_request')}}" method="post">
	                	...
	                	<button type="submit" id="friendRequestConfirmBtn{{$requester_user_data->id}}" class="btn common_btn_2 w-100">{{get_phrase('Confirm')}}</button>
	                	<button type="button" id="friendRequestAcceptedBtn{{$requester_user_data->id}}" class="btn common_btn_2  w-100 d-hidden">{{get_phrase('Accepted')}}</button>
	                </form>
	                <a href="javascript:void(0)" onclick="confirmAction('<?php echo route('profile.delete_friend_request', ['user_id' => $requester_user_data->id]); ?>', true)" class="btn del_btn d-block">{{get_phrase('Delete')}}</a>
	            </div>
	        </div>
	    </div>
	</div>
