...
use App\Models\Follower;
    ...
    $following_ids = Follower::where('follow_id', auth()->user()->id)->orderBy('created_at', 'desc')->get();
    $count = Follower::where('follow_id', auth()->user()->id)->count();
    
?>


<div class="d-flex e_media  align-items-center justify-content-between al_title mb-4">
    <h3 class="h6 fw-7 m-0" style="font-size: 11px">{{ $count . get_phrase(' People follow you') }}</h3>
</div>


@foreach($following_ids as $following_id)
    @php 
        $following_user_data = DB::table('users')->where('id', $following_id->user_id)->first();
