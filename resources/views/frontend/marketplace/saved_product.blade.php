<div class="page-wrap">
    <div class="d-md-flex pagetab-head  align-items-center justify-content-between mb-3 bg-white mb-12 radius-8 p-20">
        <h3 class="h5 pt-3"><span><i class="fa-solid fa-calendar-days"></i></span> {{ get_phrase('Marketplace') }}</h3>
        @if(auth()->user()->user_role == 'admin')
        <div class="d-flex ">
            <a href="javascript:void(0)" onclick="showCustomModal('{{route('load_modal_content', ['view_path' => 'frontend.marketplace.create_product'])}}', '{{get_phrase('Create Product')}}');" class="btn common_btn" data-bs-toggle="modal"
                data-bs-target="#createProduct" class="btn common_btn"> <i class="fa fa-plus-circle"></i>{{get_phrase('Create Products')}}</a>
            <a href="{{ route('userproduct') }}" class="btn common_btn mx-1">{{ get_phrase('My Products') }}</a>
            <a href="{{ route('product.saved') }}" class="btn common_btn" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Saved Product">{{ get_phrase('Saved') }}</a>
        </div>
        @endif
    </div>
    @if(count($saved_products)>0)
    @foreach ( $saved_products as $saved_product )
        <article class="single-entry svideo-entry bg-white p-3 e_product_image">
            <div class="row">
                ...
                <div class="col-md-7 col-lg-8 col-sm-12">
                    <div class="entry-text market_save">
                        ...
                        <div class="d-flex my-2">
                            <!-- Avatar -->
                            <div class="avatar">
                                <a href="{{ route('user.profile.view', $saved_product['user_id']) }}"><img class="h-39 avatar-img rounded-circle w-40 user_image_proifle_height" src="{{ get_user_image($saved_product->productData->getUser->photo,'optimized') }}"
                                        alt=""></a>
                            </div>
                            <div class="avatar-info ms-2">
                                <h4 class="ava-nave"><a href="{{ route('user.profile.view', $saved_product['user_id']) }}">{{  $saved_product->productData->getUser->name  }}</a></h4>
