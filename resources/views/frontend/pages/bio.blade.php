    ...
    <aside class="sidebar page_side">
        ...
        <div class="widget">
            ...
            @if ($page->user_id==auth()->user()->id)
                <button class="btn common_btn w-100 mt-8" onclick="showCustomModal('{{route('load_modal_content', ['view_path' => 'frontend.pages.edit-page-info','page_id'=>$page->id])}}', '{{get_phrase('Update Page Info')}}');">{{ get_phrase('Edit Info') }}</button>
            @endif
        </div>
        <div class="widget">
            ...
            @foreach ($suggestedpages as $suggestedpage)
                ...
                @endphp
                @if ($likecount==0)
                <div class="card n_like_image border-0 mt-3">
                    <img class="cov_img" src="{{ get_page_cover_photo($suggestedpage->coverphoto,'coverphoto') }}" alt="">
                    ...
                    @if ($likecount>0)
                        <a href="javascript:void(0)" onclick="ajaxAction('<?php echo route('page.dislike',$page->id); ?>')" class="btn btn-primary"><img class="me-1" src="{{ asset('assets/frontend/images/like-i.png') }}" alt="">{{ get_phrase('Liked') }}</a>
                    @else
                        <a href="javascript:void(0)" onclick="ajaxAction('<?php echo route('page.like',$page->id); ?>')" class="btn btn-primary"><img class="me-1" src="{{ asset('assets/frontend/images/like-i.png') }}" alt="">{{ get_phrase('Like') }}</a>
                    @endif
                </div>
                @endif
            @endforeach
