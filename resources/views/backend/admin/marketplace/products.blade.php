        ...
        <div class="col-12">
            <div class="eSection-wrap">
                <div class="row mb-3">
                    <div class="col-md-4">
                        <div class="btn-group">
                            <a href="{{ route('admin.marketplace.products') }}" class="btn btn-outline-primary {{ $page_name == 'marketplace_products' ? 'active' : '' }}">
                                {{ get_phrase('All Products') }}
                            </a>
                            <a href="{{ route('admin.marketplace.products.featured') }}" class="btn btn-outline-success {{ $page_name == 'marketplace_featured_products' ? 'active' : '' }}">
                                {{ get_phrase('Active Products') }}
                            </a>
                            <a href="{{ route('admin.marketplace.products.out-of-stock') }}" class="btn btn-outline-danger {{ $page_name == 'marketplace_out_of_stock_products' ? 'active' : '' }}">
                                {{ get_phrase('Out of Stock') }}
                            </a>
                        </div>
                    </div>
                    <div class="col-md-8">
                        <form action="{{ route('admin.marketplace.products') }}" method="GET" class="d-flex justify-content-end">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="product-image">
                                            <img src="{{ route('showProductTypeFile', ['product_type_id' => $product->id]) }}" alt="{{ $product->name }}" width="80" height="80" class="img-thumbnail"  style="object-fit: cover;">
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex flex-column">
                                            <span class="fw-bold text-dark">{{ $product->created_date }}</span>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <a href="{{ route('admin.marketplace.products.edit', $product->id) }}" class="btn btn-sm btn-primary" title="{{ get_phrase('Edit') }}">
                                                <i class="bi bi-pencil-square"></i>
                                            </a>
                                            <a href="{{ route('admin.marketplace.products.destroy', $product->id) }}" class="btn btn-sm btn-danger" onclick="return confirm('{{ get_phrase('Are you sure you want to delete this product?') }}')" title="{{ get_phrase('Delete') }}">
                ...
                @else
                    <div class="empty_box center">
                        ...
                        <br>
                        <a href="{{ route('admin.marketplace.products.create') }}" class="btn btn-primary mt-3">
