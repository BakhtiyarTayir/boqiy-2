        ...
        <div class="eSection-wrap-2">
            <div class="row">
                <div class="col-md-6 col-md-6 col-sm-6">
                    <div>
                      <form method="POST" action="{{ route('admin.update.category',$pagecategory->id) }}" enctype="multipart/form-data">
                          @csrf
                          <div class="mb-3">
                            <label for="pagecategory" class="form-label eForm-label">{{ get_phrase('Page Category') }}</label>
                            <input type="text" class="form-control eForm-control" id="pagecategory" value="{{ $pagecategory->name }}" name="pagecategory" placeholder="Page Category">
                          </div>
                          @if ($errors->any())
