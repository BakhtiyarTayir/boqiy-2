...
class MainController extends Controller
{
	...
	public function timeline()
	{
		...
		//new
		$likeBalance = $this->user->likeBalance;
		$page_data['userWallet'] = $this->user->wallet;
		$page_data['userLikeBalance'] = $likeBalance;
	...
	{
		// Check if user has given less than 3 likes
		$user_id = auth()->user()->id;
		$likesGiven = DB::table('post_likes')->where('user_id', $user_id)->count();
		
		// Check if user has at least one post
		$userPostsCount = Posts::where('user_id', $user_id)->count();
		
		// If user already has posts but less than 3 likes, show modal
		if ($userPostsCount > 0 && $likesGiven < 3) {
			return json_encode([
				'validationError' => [
					'likes' => get_phrase('You need to give at least 3 likes to other posts before posting more')
				],
				'showLikesModal' => true,
				'likesCount' => $likesGiven,
				'postsCount' => $userPostsCount
			]);
		}
		
		// Check how many posts user has created today (for payment calculation)
		$today = Carbon::today();
		$postsToday = Posts::where('user_id', $user_id)
		...
		$hasVideo = false;
		$mediaFiles = Media_files::where('post_id', $post_id)->get();
		...
		// Add wallet balance only for first 10 posts per day
		$user = auth()->user();
		$wallet = $user->wallet;
		...
		// Check if this is one of the first 10 posts today
		if ($postsToday <= 10) {
			// Add 3000 sum to wallet for any post (within first 10 daily posts)
			$wallet->deposit(
				3000.00,
				$hasVideo ? 'video_post_creation' : 'post_creation',
				$hasVideo ? 'Reward for creating a video post' : 'Reward for creating a post',
				['post_id' => $post_id]
			);
			
			// Add success message about wallet
			Session::flash('success_message', get_phrase('Your post has been published and 3000 sum added to your wallet'));
		} else {
			// No payment for posts beyond the first 10 per day
		...
		if ($data['post_type'] == 'live_streaming') {
			...
			$response = array(
				'open_new_tab' => url('/streaming/live/' . $post_id),
				'reload' => 0,
				'status' => 1,
				'function' => 0,
	...
	public function post_delete()
	{
		...
		if ($post) {
			// Check if this post has any video files
			$hasVideo = Media_files::where('post_id', $post_id)
				->where('file_type', 'video')
				->exists();
			
			// Check if this post was one of the first 10 posts of the day when it was created
			$postCreationDate = Carbon::createFromTimestamp($post->created_at);
			$startOfDay = $postCreationDate->copy()->startOfDay();
			$endOfDay = $postCreationDate->copy()->endOfDay();
			
			$postsBeforeThisOne = Posts::where('user_id', $post->user_id)
			...
			if ($done) {
				// Only deduct if it was one of the first 10 posts of the day
				if ($postsBeforeThisOne <= 10) {
					$user = User::find($post->user_id);
					$wallet = $user->wallet;
					
					if ($wallet) {
						// Deduct the same amount that was added (3000)
						$wallet->withdraw(
							3000.00,
							$hasVideo ? 'video_post_deletion' : 'post_deletion',
							$hasVideo ? 'Deduction for deleting a video post' : 'Deduction for deleting a post',
							['post_id' => $post_id]
						);
					}
				}
				...
				$response = array(
					...
					'fadeOutElem' => "#postIdentification" . $post_id
				);
			}
		}
	...
	public function unsave_post($id)
	{
		...
		$save_posts = json_decode($savedPostsJson, true);
		
		if (($key = array_search($id, $save_posts)) !== false) {
			unset($save_posts[$key]);
			$user->save_post = json_encode(array_values($save_posts));
			$user->save();
		}
	...
	public function generateImage(Request $request)
	{
		...
		try {
			...
			$response = Http::withHeaders([
				'Authorization' => env('HUGGING_FACE_API_KEY'),
	...
	public function saveProductComment(Request $request): \Illuminate\Http\JsonResponse
	{
		$productCommentData = [
			'user_id' => auth()->user()->id,
			'product_id' => $request->product_id,
			'created_date' => now()->format('Y-m-d H:i:s'),
			'type' => ProductComment::TYPE_TEXT,
		];
		...
		if ($text || $image) {
			$productCommentData['text'] = $text;
			
			if (!empty($image)) {
				$productCommentData['type'] = ProductComment::TYPE_FILE;
				$productCommentData['file_path'] = $this->saveProductCommentFile($image, $request->product_id);
			}
		}
	...
	private function saveProductCommentFile($imageFile, $productId)
	{
		$uniqueId = Str::uuid()->toString();
		
		$fileName = time() . '.' . $imageFile->getClientOriginalExtension();
		
		$path = "product/comments/{$productId}/{$uniqueId}";
	...
	public function showProductCommentFile(Request $request)
	{
		$comment = \App\Models\ProductComment::where('id', $request->comment_id)
			->where('type', \App\Models\ProductComment::TYPE_FILE)
			->first();
		...
		$path = public_path($comment->file_path);
