<?php
	...
	if (!empty($user)) {
		$blockedByUser = DB::table('block_users')->where('user_id', $user->id)->pluck('block_user')->toArray();
		$blockedByOthers = DB::table('block_users')->where('block_user', $user->id)->pluck('user_id')->toArray();
...
<style>
    .likes-panel {
        position: absolute;
        top: 100%; /* Tugmadan pastda chiqadi */
        left: 0;
        z-index: 999;
...
<style>
    ...
    .like-item span.label {
        font-size: 0.75rem;
        color: #6c757d;
    }
    
    .like-item.like-item-close {
    ...
    .like-item.like-item-close:hover {
        background-color: #f1b0b7;
        color: #fff;
        border-color: #f1b0b7;
    }
    
    .like-item.like-item-close i {
        pointer-events: none;
    }
</style>
...
<?php $__currentLoopData = $posts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $loopIndex => $post): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    // New Feature Code
			...
			if (!empty($user)) {
				 $friend = \App\Models\Friendships::where(function($query) use ($user, $user_id) {
                ...
                ->count();
				  ...
				  $friendAccepted = \App\Models\Friendships::where(function($query) use ($user, $user_id){
                        $query->where('requester', $user->id);
                        $query->where('accepter', $user_id);
                    })
                    ->orWhere(function($query) use ($user, $user_id) {
    ...
    <?php if($post->post_type != 'fundraiser'): ?>
        <div class="single-item-countable single-entry lenta-posts" id="postIdentification<?php echo e($post->post_id); ?>" data-post-id="<?php echo e($post->post_id); ?>" data-post-animation="<?php echo e($post->post_animation); ?>">
            <div class="entry-inner">
                ...
                <div class="entry-content pt-2">
                    ...
                    <?php
                        $likeBalanceCurrency = $userLikeBalance->balance ?? 0;
						$isShowLikeBalanceModal = $likeBalanceCurrency > 200;
                    ?>
                    
                    <?php if($isShowLikeBalanceModal && !empty($likes) && count($likes)): ?>
                        <div id="post-gift-like-list-<?php echo e($post->post_id); ?>" class="like-boxes d-none">
                            <?php $__currentLoopData = $likes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $like): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="like-item" onclick="sendLike(<?php echo e($post->post_id); ?>, <?php echo e($like->id); ?>, <?php echo e($like->price); ?>)">
                                    <span class="amount"><?php echo e($like->price); ?></span>
                                    <span class="label"><?php echo e($like->name); ?></span>
                                </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <div class="like-item like-item-close">
                ...
                <div class="e_comment">
                    ...
                    <div class="entry-footer">
                        ...
                        <div class="footer-share d-flex justify-content-around">
                            <!-- Gift (like) button -->
                            <?php if(!empty($user)): ?>
                                <span class="entry-react position-relative">
                                <?php if($isShowLikeBalanceModal): ?>
                                        <a href="#" class="gift-button btn-gift-like" data-gift-id="post-gift-like-list-<?php echo e($post->post_id); ?>">
                                        <span><i class="fa fa-gift fa-lg text-primary"></i></span>
                                        <?php echo e(get_phrase('Gift')); ?>

                                    </a>
                                    <?php else: ?>
                                        <a href="#" class="gift-button btn-gift-like">
                                        <span><i class="fa fa-gift fa-lg text-primary"></i></span>
                                        <?php echo e(get_phrase('Gift')); ?>
                                    ...
                                    <?php if(0): ?>
                                    <!-- Likes panel -->
                                        <div id="likes-panel-<?php echo e($post->post_id); ?>" class="likes-panel">
                                        <div class="likes-panel-header">
                                            <h5><?php echo e(get_phrase('Select a gift')); ?></h5>
                                        </div>
                                        <div class="likes-grid">
                                            ...
                                            <?php $__currentLoopData = $likes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $like): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <div class="like-item test evesrvfwevev "
                                                     onclick="sendLike(<?php echo e($post->post_id); ?>, <?php echo e($like->id); ?>, <?php echo e($like->price); ?>)"
                                                >
                                                    <span class="like-price"><?php echo e($like->price); ?></span>
                                                </div>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        </div>
                                    </div>
                                ...
                                <span class="entry-react no-login-post-comment">
                                <a href="javascript:void(0)"
                                   onclick="$('#user-comments-<?php echo e($post->post_id); ?>').toggle();">
                                ...
                                <a href="javascript:void(0)"
                                   onclick="myReact('post', 'like', 'toggle', <?php echo e($post->post_id); ?>)"
                                   id="my_post_reacts<?php echo e($post->post_id); ?>">
                                ...
                                <li><a href="javascript:void(0)"
                                       onclick="myReact('post', 'like', 'update', <?php echo e($post->post_id); ?>)">
                                        <?php if(0): ?>
                                            <img src="<?php echo e(asset('storage/images/like.svg')); ?>" alt="Like" >
                                        <?php endif; ?>
                                        <span style="margin-right: 1px;"> <i class="far fa-thumbs-up"></i></span>
                                    </a>
                                </li>
                                <li><a href="javascript:void(0)" onclick="myReact('post', 'love', 'update', <?php echo e($post->post_id); ?>)">
                                        <img src="<?php echo e(asset('storage/images/love.svg')); ?>" alt="Love" style="width: 30px; margin-top: 2px;"></a>
                                </li>
                                <li><a href="javascript:void(0)"
                                       onclick="myReact('post', 'haha', 'update', <?php echo e($post->post_id); ?>)"><img
                                            src="<?php echo e(asset('storage/images/haha.svg')); ?>" alt="Haha"></a>
                                </li>
                                <li><a href="javascript:void(0)"
                                       onclick="myReact('post', 'sad', 'update', <?php echo e($post->post_id); ?>)"><img
                                            src="<?php echo e(asset('storage/images/sad.svg')); ?>" class="mx-1"
                                            alt="Sad"></a>
                                </li>
                                <li>
                                    <a href="javascript:void(0)" onclick="myReact('post', 'angry', 'update', <?php echo e($post->post_id); ?>)">
                                        <img src="<?php echo e(asset('storage/images/angry.svg')); ?>" alt="Angry"></a>
                                </li>
                            </ul>
                            </span>
                                
                                <?php if(!isset($has_memories)): ?>
                                    <span class="entry-react no-login-post-share" data-bs-toggle="modal" data-bs-target="">
                                <a href="javascript:void(0)"
                                   onclick="showCustomModal('<?php echo e(route('load_modal_content', ['view_path' => 'frontend.main_content.share_post_modal', 'post_id' => $post->post_id])); ?>', '<?php echo e(get_phrase('Share post')); ?>');">
                                ...
                                <?php endif; ?>
                            <?php else: ?>
                                <span style="margin-right: 1rem; cursor: pointer" class="no-login-post-gift">
                                <i class="fa fa-gift fa-lg text-primary"></i>
                                <?php echo e(get_phrase('Gift')); ?>

                            </span>
                                <span style="margin-right: 1rem; cursor: pointer" class="no-login-post-like">
                                <i class="far fa-thumbs-up fa-lg text-primary2"></i>
                                <?php echo e(get_phrase('Like')); ?>

                            </span>
                                <span style="margin-right: 1rem; cursor: pointer" class="no-login-post-comment">
                                <i class="far fa-comment-alt fa-lg"></i>
                                <?php echo e(get_phrase('Comments')); ?>

                            </span>
                                <span style="margin-right: 1rem; cursor: pointer" class="no-login-post-share">
                        ...
                        <div class="entry-meta py-2 d-flex justify-content-between align-items-center">
                            <a href="javascript:void(0)" id="post_reacts<?php echo e($post->post_id); ?>">
                                ...
                                <ul>
                                    ...
                                    <?php
                                        $sharecount = \App\Models\Post_share::where('post_id', $post->post_id)
											...
											->count();
                                    ?>
                                    <li><a href="javascript:void(0)"><span> <?php echo e($sharecount); ?>

                                        </span><?php echo e(get_phrase('Share')); ?></a></li>
                                </ul>
                            </div>
                        </div>
                ...
                <?php if(isset($has_memories)): ?>
                    ...
                    <span class="entry-react eshare" data-bs-toggle="modal" data-bs-target="">
                        <a href="javascript:void(0)"
                           ...
                           onclick="showCustomModal('<?php echo e(route('load_modal_content', ['view_path' => 'frontend.main_content.share_post_modal', 'post_id' => $post->post_id, 'is_memory' => $post->post_id])); ?>', '<?php echo e(get_phrase('Share post')); ?>');">
            ...
            <div class="user-comments s_comment d-hidden bg-white" id="user-comments-<?php echo e($post->post_id); ?>">
                ...
                <div class="comment-form d-flex pb-3 align-items-start">
                    <?php if(!empty($user)): ?>
                        <img src="<?php echo e(get_user_image($user->photo, 'optimized')); ?>" alt=""
                             class="rounded-circle img-fluid h-39" width="40px">
                    <?php endif; ?>
                    <form action="javascript:void(0)" class="w-100 ms-2 d-flex align-items-center position-relative post-form" method="post">
                        ...
                        <span type="button" class="position-absolute end-0"
                              style="top: 50%; transform: translateY(-50%); margin-right: 0.8rem"
                              onclick="postComment(this, 0, <?php echo e($post->post_id); ?>, 0,'post', true);" rows="1">
                        <i class="fas fa-paper-plane text-primary fa-xs2"></i>
                    </span>
                    </form>
                </div>
            </div>
...
<?php echo $__env->make('modals.redirectLikeBalanceModal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
<?php /**PATH /home/asilbek/Downloads/boqiy-uz/boqiy.uz-master/web/resources/views/frontend/main_content/posts.blade.php ENDPATH**/ ?>