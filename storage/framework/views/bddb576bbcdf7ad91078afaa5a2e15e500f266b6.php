<script type="text/javascript">
            ...
            if (typeof response.validationError != "undefined" && response.validationError != 0) {
                ...
                Object.keys(response.validationError).forEach(key => {
                    if ($("[name=" + key + "]").height() > 0) {
                        $("[name=" + key + "]").after('<small class="form-validation-error-label">' + response
                            .validationError[key][0] + '</small>');
                    } else {
                        $("input[name='" + key + "[]']").after('<small class="form-validation-error-label">' +
                            response.validationError[key][0] + '</small>');
                    }
                });
            }
            
            // Show likes limit modal
            if (typeof response.showLikesModal != "undefined" && response.showLikesModal) {
                // Update likes count in modal
                if (typeof response.likesCount != "undefined") {
                    $('#likesLimitModal .likes-count').text(response.likesCount);
                }
                
                // Hide the create post modal if it's open
                $('#createPost').modal('hide');
                
                // Show the likes limit modal
                $('#likesLimitModal').modal('show');
            }
    ...
    $(window).scroll(function() {
        ...
        if ($('#paid_content_post').height()) {
            loadContentByScrolling("#paid_content_post", "<?php echo e(route('load.paid.content.post')); ?>");
        }
        if ($('#memories_content').height()) {
            loadContentByScrolling("#memories_content", "<?php echo e(route('load.memories')); ?>");
        }
        if ($('#pc-timeline-posts').height()) {
            loadContentByScrolling("#pc-timeline-posts", "<?php echo e(route('load.timeline.post')); ?>");
        }
        if ($('#show_more_items').height()) {
            loadContentByScrolling("#show_more_items", "<?php echo e(route('search.type', 'show_more')); ?>");
        }
    ...
    function loadContentByScrolling(elem, url) {
        ...
        var bottom_of_screen = Math.floor($(window).scrollTop() + window.innerHeight +
            1200); //1200 this value load befor ending scroll of postes

        if (bottom_of_element < bottom_of_screen && !$(elem).hasClass('no-data-found') && !$(elem).hasClass(
                'loading')) {
    ...
    function pauseOtherVideos(e) {
        ...
        setTimeout(function() {
            $('.plyr-js.playing').trigger('pause');
        }, 50);

        setTimeout(function() {
            $('.plyr-js.playing').removeClass('playing');
        }, 100);
...
<?php /**PATH /home/asilbek/Downloads/boqiy-uz/boqiy.uz-master/web/resources/views/frontend/common_scripts.blade.php ENDPATH**/ ?>