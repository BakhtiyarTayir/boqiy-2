...
if (!function_exists('get_phrase')) {
    function get_phrase($phrase = '', $value_replace = array())
    {
	    ...
	    // @todo: Asilbek translatelarni DB saqlash shartmi
        ...
        if ($query->count() > 0) {
            ...
            if ($all_language->count() > 0) {
                ...
                DB::table('languages')->insert(array('name' => 'uz', 'phrase' => $phrase, 'translated' => $phrase));
            }
        }
...
if (!function_exists('get_system_logo')) {
	function get_system_logo($file_name = '', $foldername = 'boqiyUz')
	{
		//this file comes from another online link as like amazon s3 server
		if (strpos($file_name, 'https://') !== false) {
			return $file_name;
